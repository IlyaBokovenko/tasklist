...
- (void)insertObject:(NSManagedObject*)object inOrderedValueForKey:(NSString*)key atIndex:(NSUInteger)index
{
	...
	NSURL* rep = [[object objectID] URIRepresentation];
	[newOrdering insertObject:rep atIndex:index];
...
{
	NSMutableArray* newOrdering = [[self orderingForKey:key] mutableCopy];
	NSManagedObjectID* oldObjectID = [[[self managedObjectContext]
									   persistentStoreCoordinator] 
										managedObjectIDForURIRepresentation:
											[newOrdering objectAtIndex:index]];
	NSManagedObject* oldObject = [[self managedObjectContext] objectRegisteredForID:oldObjectID];
	
	[newOrdering removeObjectAtIndex:index];
	[self setOrdering:newOrdering forKey:key];
	if(![newOrdering containsObject: [oldObjectID URIRepresentation]]) [[self mutableSetValueForKey:key] removeObject:oldObject];
	[newOrdering release];
}
...
- (void)replaceObjectInOrderedValueForKey:(NSString*)key atIndex:(NSUInteger)index withObject:(NSManagedObject*)newObject
{
	...
	[newOrdering replaceObjectAtIndex:index withObject: [[newObject objectID] URIRepresentation]];
	[self setOrdering:newOrdering forKey:key];
	if(![newOrdering containsObject: [oldObjectID URIRepresentation]]) [[self mutableSetValueForKey:key] removeObject:oldObject];
